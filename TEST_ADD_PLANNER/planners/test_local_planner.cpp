...
// Utils
void print_situation2(const colregs_t2 &sit);
void get_state_str2(std::string &str, const colregs_t2 &state);
void rot2d2(const Eigen::Vector2d &v, double yaw, Eigen::Vector2d &result);
void normalize_angle2(double &alpha);
void normalize_angle_diff2(double &angle, const double &angle_ref);

ExerciseLocalPlanner::ExerciseLocalPlanner() : EDGE_SAMPLES_(10),
                                       VEL_SAMPLES_(41),
                                       ANG_SAMPLES_(101),
                                       //RADIUS_(10.0),
                                       //MAX_VEL_(20.),
...
void ExerciseLocalPlanner::init(std::vector<asv_msgs::State> *obstacles,
...
void ExerciseLocalPlanner::update_lp()
{
  clearVelocityGrid();
...
void ExerciseLocalPlanner::update_VelocityGrid()
{

  if (state_list_.size() < obstacles_->size()){
    state_list_.resize(obstacles_->size(), NO_SITUATION2);
  }
  ...
  Eigen::Vector2d va;
  rot2d2(asv_twist_.head(2), asv_pose_[2], va);
  ...
  for (it = obstacles_->begin(); it != obstacles_->end(); ++it) {
    ...
    Eigen::Vector2d vb;
    rot2d2(obstacle_twist.head(2), obstacle_pose[2], vb);
    ...
    normalize_angle_diff2(angle_diff, asv_pose_[2]);

    bool collision_situation    = inCollisionSituation(asv_pose_, obstacle_pose, va, vb);
    colregs_t2 colregs_situation = inColregsSituation(bearing, angle_diff);
    ...
    if (it->header.prior == "give_way")
      ...
      apply_colregs = (colregs_situation == HEAD_ON2 ||
                       colregs_situation == CROSSING_RIGHT2 ||
                       colregs_situation == OVERTAKING2) && collision_situation;



    /// @todo Move this to own function, e.g., this->handleEvent(...);
    colregs_t2 prev_state = state_list_[i];
    ...
    if (state_list_[i] != prev_state) {
      ...
      get_state_str2(pstate, prev_state);
      get_state_str2(nstate, state_list_[i]);
    ...
    pab = pab/pab_norm;
    rot2d2(pab,  alpha - 0.5*M_PI, lb);
    rot2d2(pab, -alpha + 0.5*M_PI, rb);
    ...
    for (int u_it=0; u_it<VEL_SAMPLES_; ++u_it) {
      for (int t_it=0; t_it<ANG_SAMPLES_; ++t_it) {
        ...
        if (collision_situation && inExerciseLocalPlanner(u, t, lb, rb, vb))
          {
            setVelocity(u_it, t_it, VELOCITY_NOT_OK2);
          }
        else if ((apply_colregs &&
                  violatesColregs(u, t, obstacle_pose, vb)))
          {
            setVelocity(u_it, t_it, VELOCITY_VIOLATES_COLREGS2 + objval/2.0);
          }
        else
          {
            /// ALREADY SET
...
void ExerciseLocalPlanner::checkStaticObstacles()
{
      ...
      if (velocity_ok)
        {
          ...
          for (int i=u_it; i >= 0; --i)
            setVelocity(i, theta_it, VELOCITY_OK2);
          break;
        }
      else
        {
          setVelocity(u_it, theta_it, VELOCITY_NOT_OK2);
        }
...
bool ExerciseLocalPlanner::inObstacle(double px, double py)
{
  ...
  return map_->data[px_i + py_i*map_->info.width] > OCCUPIED_TRESH2;
}

bool ExerciseLocalPlanner::inExerciseLocalPlanner(const double &u,
                                          const double &theta,
...
bool ExerciseLocalPlanner::violatesColregs(const double &u,
                                       const double &theta,
...
void ExerciseLocalPlanner::clearVelocityGrid()
{
  for (int i=0; i<vo_grid_.size(); ++i)
    vo_grid_[i] = 0.0;
}

void ExerciseLocalPlanner::setVelocity(const int &ui, const int &ti, const double &val)
{
...
void ExerciseLocalPlanner::initMarker(visualization_msgs::Marker *marker)
{
  marker_ = marker;
  double u0 = 0, u = 0;
...
void ExerciseLocalPlanner::getBestControlInput(double &u_best, double &psi_best)
{
...
bool ExerciseLocalPlanner::inCollisionSituation(const Eigen::Vector3d &pose_a,
...
colregs_t2 ExerciseLocalPlanner::inColregsSituation(const double &alpha,
...
void print_situation2(const colregs_t2 &sit) {
  ...
  case OVERTAKING2:
    ...
    break;
  default:
    ROS_INFO("wtf: %d, %d", sit, OVERTAKING2);
    break;
  }
}
