...
ExerciseGlobalPlanner::ExerciseGlobalPlanner() :  X_STEP_(2), Y_STEP_(2) {}

ExerciseGlobalPlanner::~ExerciseGlobalPlanner() {}

void ExerciseGlobalPlanner::init(nav_msgs::OccupancyGrid *map) {
  map_ = map;
...
void ExerciseGlobalPlanner::reinit() {
  for (int x = 0; x < (X_MAX_ / X_STEP_); x++) {
    for (int y = 0; y < (Y_MAX_ / Y_STEP_); y++) {
      int id = x + y * (X_MAX_ / X_STEP_);
...
bool ExerciseGlobalPlanner::isValid(int x, int y) { //If our Node is an obstacle it is not valid
...
bool ExerciseGlobalPlanner::isObstacle(int x, int y)
{
  int count = 0;
...
bool ExerciseGlobalPlanner::isDestination(int x, int y, Node2 dest) {
...
double ExerciseGlobalPlanner::calculateH(int x, int y, Node2 dest) {
  double H = sqrt((x - dest.x)*(x - dest.x) + 0.2*(y - dest.y)*(y - dest.y));
...
asv_msgs::Path ExerciseGlobalPlanner::aStar(Node2 player, Node2 dest) {
  asv_msgs::Path empty;
  ...
  while (!openList.empty()&&openList.size()<(X_MAX_ / X_STEP_)*(Y_MAX_ / Y_STEP_)) {
    ...
    do {
      ...
      float temp = FLT_MAX;
      std::vector<Node2>::iterator itNode;
      for (std::vector<Node2>::iterator it = openList.begin(); it != openList.end(); it = next(it)) {
        Node2 n = *it;
        if (n.fCost < temp) {
          temp = n.fCost;
          itNode = it;
...
asv_msgs::Path ExerciseGlobalPlanner::makePath(Node2 dest) {
  //try {
    ROS_INFO("GLOBAL PLANNER SUCCESS : Found a path");
    int x = dest.x;
    int y = dest.y;
    int id = x + y*(X_MAX_ / X_STEP_);
    std::stack<Node2> path;
...
bool ExerciseGlobalPlanner::thereIsAWay(Node2 n_inf, Node2 n_sup)
{
  int x_inf = n_inf.x;
  int y_inf = n_inf.y;
  int x_sup = n_sup.x;
...
asv_msgs::Path ExerciseGlobalPlanner::get_path(const double start_x, const double start_y, const double arrival_x, const double arrival_y)
{
...
inline bool operator < (const Node2& lhs, const Node2& rhs)
