...
class Fig(object):
    ...
    def __init__(self, input, output):
        ...
        for line in f1:
            ...
            if content[1] == "OVERTAKING":
                ...
                raise("Unexpected")
        self.secu = ['Safe', 'Not quite Safe', 'Not quite Dangerous', 'Dangerous']
        self.perf = ['Fast', 'Slow']
        self.total = len(self.idata)
    ...
    def set_security_and_performances_classes(self):
        ...
        print(' ')
        df = pd.DataFrame(self.security_and_performances_classes,columns=self.perf,index=self.secu)
        print(df)
        print(' ')
...
def gui(name):
    ...
    def update_plot():
        ...
        graph_fig.x = x.get()
        graph_fig.y = y.get()
        graph_fig.disp_sit = [bool(sits[k].get()) for k in range(5)]
    ...
    list = [["Opus",0], ["ASV Speed",1], ["Obstacle Heading",2], ["Obstacle Speed",3], ["Theoretical dCPA",4], ["Detection Distance",5]]
    ...
    txt = ["Display Overtaking", "Display Overtaken", "Display Crossing right", "Display Crossing left", "Display Head on"]
    for k in range(5):
        tk.Checkbutton(frame3, text=txt[k], variable=sits[k]).pack()
