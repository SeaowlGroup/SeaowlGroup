...
def run(serial, input_file, opus, param, uuid) :
    # ASV parameters
...
if __name__ == "__main__":
    ...
    # run(serial, f, opus, params[opus], uuid)
    print(params[opus])
    p1 = Process(target=run, args=(serial, f, opus, params[opus], uuid))
    opus+=1
    p2 = Process(target=run, args=(serial, f, opus, params[opus], uuid))
    p1.start()
    p2.start()
    p1.join()
    p2.join()
