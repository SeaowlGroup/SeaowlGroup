...
def gui(name, input, output):
    ...
    for g in range(graph_fig.n_groups):
        ...
        groups[g].set(1)
        g_to_indic = ['safe', 'insecure', 'dangerous']
        tk.Checkbutton(frame3, text=f"Display {g_to_indic[g+1]} opuses", variable=groups[g], command=update_plot).pack()
    ...
    if filepath:
        ...
        for line in f2:
            ...
            if float(content[4]) > 0.: # dangerous security indicator
                ...
                group = 1
            f3.write(f1.readline()[:-2]+f'{group}\n')

        # gui(name, input, output)
    else:
